...
- **Параметр `?version=working`** для автора — по slug не поддерживается (slug — публичный URL). Для чтения по Id реализован частично: UC-DSH-050 v1.3 и UC-DSH-052 принимают `?version=working` (автор/admin). Полный UC-DSH-083 (представление рабочей версии, откат правок) — Этап 5 или 8+.
