...
**Version:** 1.0 (MVP — без проверки подписок; Premium-проверка появится на Этапе 3+) | **Date:** 2026-06-07
...
На Этапе 2 — нет. На Этапе 3+ добавится проверка через `ISubscriptionService`: рецепты помеченных Premium-блюд доступны только подписчикам Premium+ или автору блюда. Конкретный механизм маркировки блюда как Premium — отдельное решение Этапа 3+.
...
- Никаких побочных эффектов: счётчик просмотров не инкрементируется (это задача UC-DSH-070 на стороне UC-DSH-050).
...
- **Premium-проверка через `ISubscriptionService`** — Этап 3+. Сейчас доступ предоставляется всем `VALID_ACTOR`.
