...
**Version:** 1.2 (+`?version=working` — рабочая версия для автора/admin, частичная реализация UC-DSH-083) | **Date:** 2026-08-01
...
- `version` — опционально. `working` — отдать рабочую версию рецепта вместо снепшота (источник данных редактора рецепта при опубликованном блюде). Учитывается **только** для автора/admin; для остальных игнорируется (Premium-гейт по снепшоту работает как без параметра). Частичная реализация UC-DSH-083.
...
- ~~**Параметр `?version=working`** для явного запроса рабочей версии автором~~ — реализовано (v1.2). Остаток UC-DSH-083 (откат несохранённых правок к снепшоту, строка-черновик в списках) — Этап 5+.
