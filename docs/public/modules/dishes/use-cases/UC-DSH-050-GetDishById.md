...
**Version:** 1.3 (+`?version=working` — рабочая версия для автора/admin, частичная реализация UC-DSH-083) | **Date:** 2026-08-01
...
- `version` — опционально. `working` — отдать рабочую версию вместо снепшота (источник данных редактора карточки). Учитывается **только** для автора/admin; для остальных пользователей параметр игнорируется (гость получает снепшот, как без параметра). Частичная реализация отложенного UC-DSH-083.
...
- Никаких побочных эффектов: счётчик просмотров не инкрементируется из этого UC. UC-DSH-070 IncrementDishViews — отдельный публичный эндпоинт `POST /api/dishes/{id}/views`; пинг шлёт клиент после успешного рендера карточки. Это позволяет фильтровать SSR/пререндер и оставить UC-050 чистой read-only Query.
...
- Нет — UC-DSH-070 IncrementDishViews реализован как отдельный публичный эндпоинт; UC-050 не отвечает за инкремент просмотров (клиент шлёт пинг сам после рендера).
