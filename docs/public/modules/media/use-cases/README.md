...
> **Версия:** 1.1 (Этап 2 — UC-MED-001..005, 101..103, 200..204 реализованы)
> **Последнее изменение:** 2026-06-06
> **Статус:** Этап 2 в работе (Core-UC реализованы, фоновые задачи и UC-MED-006 — Drafted)
...
| Статус | Значение |
|--------|----------|
| **Core (реализовано)** | Реализован на Этапе 2 |
| **Core** | Реализуем на Этапе 2 (ещё не реализован) |
...
|----|----------|-----|--------|------|----------|
| UC-MED-001 | Загрузить файл (пользователь) | Cmd | Core (реализовано) | 2 | JPEG/PNG, до 10 MB, 100×100 — 4096×4096 px |
| UC-MED-002 | Получить файл по ID | Qry | Core (реализовано) | 2 | Стриминг через контроллер |
| UC-MED-003 | Получить миниатюру файла | Qry | Core (реализовано) | 2 | По размеру и формату |
| UC-MED-004 | Получить метаданные файла | Qry | Core (реализовано) | 2 | Без содержимого |
| UC-MED-005 | Удалить свой файл (пользователь) | Cmd | Core (реализовано) | 2 | Soft delete; только владелец |
| UC-MED-006 | Получить мои файлы | Qry | Drafted | 8+ | Список + сводный объём |
| UC-MED-101 | Загрузить системный файл (admin) | Cmd | Core (реализовано) | 2 | Иконки категорий, фото ингредиентов; SVG разрешён |
| UC-MED-102 | Удалить любой файл (admin) | Cmd | Core (реализовано) | 2 | Soft delete без проверки владельца |
| UC-MED-103 | Получить файлы пользователя (admin) | Qry | Core (реализовано) | 2 | Аудит, экспорт по 152-ФЗ |
| UC-MED-200 | Получить метаданные файла (внутренний) | Internal | Core (реализовано) | 2 | `IMediaService.GetMetadataAsync` |
| UC-MED-201 | Batch-получение метаданных (внутренний) | Internal | Core (реализовано) | 2 | `IMediaService.GetMetadataBatchAsync` |
| UC-MED-202 | Привязать файл к сущности (внутренний) | Internal | Core (реализовано) | 2 | `IMediaService.AttachToEntityAsync` |
| UC-MED-203 | Отвязать файл от сущности (внутренний) | Internal | Core (реализовано) | 2 | `IMediaService.DetachFromEntityAsync` |
| UC-MED-204 | Удалить файлы сущности (внутренний) | Internal | Core (реализовано) | 2 | `IMediaService.DeleteByEntityAsync` |
...
**Доступ (Этап 2):**
- `DataCategory = Public` (Dish.MainImage, RecipeStep.Image, CategoryIcon, IngredientImage) — отдаём всем, включая **гостей**. Для RecipeStep.Image проверка гранта через `ISubscriptionAccessService` (POL-004) пока не подключена: изображения шагов отдаются без учёта подписки, хотя сам текст рецепта уже закрыт грантом `FullRecipes` (UC-DSH-052).
