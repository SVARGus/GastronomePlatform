...
> **Дата:** 2026-04-19 (создано), 2026-04-26 (расширено), 2026-05-10 (отметки реализации), 2026-05-23 (синхронизация с реализацией Этапа 2)
...
>
> **Изменения от 2026-05-23 (синхронизация с реализацией Этапа 2):**
> - `AllergensMask` перенесён с `Recipe` на `Dish` как корень агрегата. Добавлено новое поле `HasUnverifiedAllergens` для дисклеймера про freeform-ингредиенты. Метод `RecalculateAllergens(...)` тоже перенесён на `Dish`. Обоснование: денормализованные публичные маркеры используются каталожным фильтром UC-DSH-054 — на корне доступ дешевле, симметрия с `RatingAvg`/`FavoritesCount`/`ViewsCount`.
> - Уточнён список исключаемых полей `Dish` в `SaveChangesInterceptor` — добавлено `UpdatedAt` (чтобы поле само себя не триггерило). Добавлены реализационные детали: skip Added/Deleted, поиск Dish через ChangeTracker без обращения к БД.
> - Уточнены сигнатуры `Dish.Create(...)` и `Dish.Publish(...)` в соответствии с реализацией: явный `slug`, обязательные `difficultyLevel`/`costEstimate`, снепшот передаётся готовой строкой из Application.
> - Добавлена явная заметка про DDD-принцип в разделе Recipe: методы `Recipe` помечены `internal`, внешний код взаимодействует с агрегатом только через wrapper-методы на `Dish`.
...
|---|-------------|---------------|--------|----------|---------------|-------|
| 1 | Блюдо | `Dish` | 🟢 | Корень агрегата. Публичная карточка блюда | Id, AuthorUserId, Name, Slug, Description, MainImageId, Status, ModerationStatus, DifficultyLevel, CostEstimate, OwnerType, DietLabelsMask, **AllergensMask**, **HasUnverifiedAllergens**, HistoryText, RatingAvg, RatingCount, ViewsCount, FavoritesCount, **PublishedVersionData**, **PublishedVersionUpdatedAt**, PublishedAt, CreatedAt, UpdatedAt |
| 2 | Рецепт | `Recipe` | 🟢 | Инструкция приготовления. Часть агрегата `Dish` | Id, DishId, IntroductionText, ServingsDefault, IsAlcoholic, AuthorTips, ServingSuggestions, Notes |
...
- `Dish` (но без полей `PublishedVersionData`, `PublishedVersionUpdatedAt`, `RatingAvg`, `RatingCount`, `ViewsCount`, `FavoritesCount`, `UpdatedAt` — они меняются по другим причинам, плюс сам `UpdatedAt` исключён, чтобы поле не триггерило само себя)
...
**Реализационные детали (`UpdatedAtInterceptor`):**

- Интерсептор пропускает состояния `EntityState.Added` (новое блюдо — `UpdatedAt` уже задан в конструкторе) и `EntityState.Deleted` (блюдо удаляется — обновлять нечего) для самой сущности `Dish`.
- Поиск родительского `Dish` для дочерних сущностей (`Recipe`, `RecipeStep`, `RecipeIngredient`, `Timing`, `Yield`) выполняется через `ChangeTracker.Entries<Recipe>()` **без обращения к БД**. Если родительская сущность не загружена в трекер (нетипичный сценарий), обновление `UpdatedAt` не выполняется.
- Для `Nutrition` ищется владеющий `Recipe` по совпадению `Recipe.NutritionId` — это автоматически отсеивает случай, когда `Nutrition` принадлежит другой сущности (например, `Ingredient.DefaultNutrition`).
- Установка `UpdatedAt` выполняется напрямую через `dishEntry.Property(...).CurrentValue = utcNow`, без вызова Domain-метода `MarkAsUpdated()` — чтобы не дублировать `DishUpdatedEvent` (его поднимают wrapper-методы Dish, через которые и пришло основное изменение).
...
| `AllergensMask` | `AllergenType` | int | NOT NULL, default 0 | Битовая маска. **Денормализовано** на корне агрегата для каталожного фильтра UC-DSH-054. Пересчитывается из `RecipeIngredient → Ingredient.AllergenType` при изменении состава ингредиентов через `Dish.RecalculateDishMarkers(...)`, вручную не задаётся |
| `HasUnverifiedAllergens` | `bool` | boolean | NOT NULL, default false | `true`, если в рецепте есть хотя бы один `RecipeIngredient` со свободным текстом (`FreeformText`, без ссылки на справочник). Сигнализирует, что `AllergensMask` может быть неполной. UI показывает дисклеймер «Может содержать и другие аллергены — уточняйте у автора» |
...
- **`AllergensMask` и `HasUnverifiedAllergens` пересчитываются только** через `RecalculateDishMarkers(...)`, не задаются напрямую. `HasUnverifiedAllergens = true` ⇔ существует хотя бы один `Recipe.Ingredients[i].FreeformText != NULL`. Тот же метод автокорректирует `DietLabelsMask` по `Ingredient.DietConflictsMask` (ADR-0016).
...
| `UpdateCard(name, shortDesc, desc, difficulty, cost, ownerType, utcNow)` | Реализовано | Обновление полей публичной карточки (`Name`, `ShortDescription`, `Description`, `DifficultyLevel`, `CostEstimate`, `OwnerType`). `UpdatedAt = utcNow`. Поднимает `DishUpdatedEvent`. **Не** включает `DietLabelsMask`, `MainImageId`, `HistoryText` — они меняются отдельными методами (см. ниже) с разной семантикой инвариантов |
...
| `SetDietLabels(DietLabels mask, IReadOnlyDictionary<Guid, DietLabels> ingredientConflicts, DateTimeOffset utcNow): Result` | Реализовано | Устанавливает битовую маску диетических меток. Reject-семантика по ADR-0016: если `mask AND combinedConflictsMask != 0` — возвращает `DietLabelsConflictWithComposition` (409). Иначе присваивает маску, обновляет `UpdatedAt`, поднимает `DishUpdatedEvent`. Application Handler собирает словарь конфликтов через `IIngredientRepository.GetMarkersByIdsAsync`. Freeform-ингредиенты в проверке не участвуют |
...
| ~~`IncrementViews()`~~ | Не реализуется как доменный метод | UC-DSH-070 реализован через прямой `UPDATE` в `IDishRepository.IncrementViewsAsync` (EF Core 8 `ExecuteUpdateAsync`) без загрузки агрегата. Доменный метод не нужен: у инкремента нет инвариантов, требующих защиты в Domain (CHECK-constraint `>= 0` в БД обеспечивает физическую целостность; +1 его не нарушает). См. UC-DSH-070 |
...
| `RecalculateDishMarkers(IReadOnlyDictionary<Guid, IngredientMarkers> markers, DateTimeOffset utcNow)` | Реализовано | Полностью пересобирает `AllergensMask`, `HasUnverifiedAllergens` и автокорректирует `DietLabelsMask` (auto-clear конфликтующих бит) из текущего состава `Recipe.Ingredients`. Application Handler собирает словарь через `IIngredientRepository.GetMarkersByIdsAsync(...)`. При снятии хотя бы одной диет-метки поднимается `DishDietLabelsAutoCorrectedEvent`. Вызывается из UC-DSH-030, 031, 032 после изменения состава. Реализует ADR-0016 |
...
| `MarkAsUpdated(DateTimeOffset utcNow)` | Реализовано | Public-метод: ставит `UpdatedAt = utcNow` и поднимает `DishUpdatedEvent`. Используется в `SetCategories`/`SetTags`, во всех wrapper-методах для Recipe, в `RecalculateDishMarkers` |
...
- **`DietLabels` — constrained declaration, не часть карточки.** Маска вынесена из `UpdateCard` в отдельный метод `SetDietLabels` с проверкой совместимости с составом рецепта (ADR-0016). Реализация состоит из трёх компонентов:
  1. **Справочник `Ingredient.DietConflictsMask`** — битовая маска `DietLabels`, с которыми ингредиент конфликтует. Заполняется модератором через UC-DSH-110/111. Default `None`.
  2. **Reject-валидация в `SetDietLabels`** — Domain собирает `combinedConflictsMask` по catalog-ингредиентам рецепта и возвращает `DietLabelsConflictWithComposition` при пересечении с запрошенной маской.
  3. **Auto-clear в `RecalculateDishMarkers`** — при изменении состава из `DietLabelsMask` снимаются конфликтующие биты; если хотя бы один бит снят — поднимается `DishDietLabelsAutoCorrectedEvent`.

  Freeform-ингредиенты не имеют справочной маски и в проверке не участвуют (ответственность автора). Подробности — `docs/public/adr/ADR-0016-diet-conflicts-mask.md` и `use-cases/UC-DSH-009-SetDietLabels.md`.
