...
| `AllergensMask` | `AllergenType` | int | NOT NULL, default 0 | Битовая маска. **Денормализовано** на корне агрегата для каталожного фильтра UC-DSH-054. Пересчитывается из `RecipeIngredient → Ingredient.AllergenType` при изменении состава ингредиентов через `Dish.RecalculateAllergens(...)`, вручную не задаётся |
...
- **`AllergensMask` и `HasUnverifiedAllergens` пересчитываются только** через `RecalculateAllergens(...)`, не задаются напрямую. `HasUnverifiedAllergens = true` ⇔ существует хотя бы один `Recipe.Ingredients[i].FreeformText != NULL`.
...
| `SetDietLabels(DietLabels mask, DateTimeOffset utcNow)` | Реализовано (заглушка) | Устанавливает битовую маску диетических меток. Сейчас — простая запись значения + `UpdatedAt`. **Полноценная валидация по составу ингредиентов** (например, запрет `Vegan` при наличии мясного ингредиента) и **автокоррекция при изменении состава** появятся позже — требуется справочник `Ingredient.DietConflictsMask` и связанная логика в `Recipe.*Ingredient*`. См. заметку про DietLabels ниже. Поднимает `DishUpdatedEvent` |
...
- **`DietLabels` — constrained declaration, не часть карточки.** Маска вынесена из `UpdateCard` в отдельный метод `SetDietLabels`. Семантическое основание: остальные поля карточки — простые атрибуты, описываемые автором без внешних ограничений; диет-метки же должны проверяться по составу ингредиентов (нельзя поставить `Vegan` блюду с мясом). Часть меток (`Vegan`, `Vegetarian`, `LactoseFree`, `GlutenFree`) — частично выводимы из состава ингредиентов, часть (`Halal`, `Kosher`) — чисто декларативные. Сейчас валидации по составу ещё нет (заглушка), реализация требует:
  1. Справочник `Ingredient.DietConflictsMask` — с какими `DietLabels` конфликтует ингредиент (новое поле или derived из `AllergenType` + категории — открытый архитектурный вопрос, кандидат на ADR).
  2. Валидацию в `SetDietLabels` — текущий состав ингредиентов должен быть совместим с устанавливаемой маской.
  3. Автокоррекцию `DietLabelsMask` в `Recipe.*Ingredient*`-методах: при изменении состава снимать биты, конфликтующие с новыми ингредиентами. Поднимать отдельное доменное событие об автокоррекции.
