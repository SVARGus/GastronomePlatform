...
> **Дата:** 2026-04-20 (создано), 2026-05-02 (расширено)
> **Связанные документы:** [[05_Дорожная-карта]], [[02_Архитектура]], [[13_Структура-проекта]], [[08_Разработка-(Development-Guide)]], `domain-model.md` (модуль Dishes), `POL-002-media-access.md`, `POL-003-media-ownership.md`
>
> **Изменения от 2026-05-02:**
> - Введены ссылки на политики авторизации `POL-002 Media Access Policy` (для UC чтения файлов) и `POL-003 Media Ownership Policy` (для UC модификации файлов).
> - Добавлен новый концептуальный раздел «Жизненный цикл `MediaFile`» с описанием статусов и переходов.
> - Расширены архитектурные решения: разграничение доступа Public/Personal, разделение Owner/Admin для системных файлов.
> - Уточнена документация по UC-MED-202 (`AttachToEntityAsync`) — описана связь с межмодульной авторизацией.
...
5. [Жизненный цикл `MediaFile`](#5-жизненный-цикл-mediafile)
6. [Enums](#6-enums)
...
14. [Структура файлов на диске и в S3](#14-структура-файлов-на-диске-и-в-s3)
15. [Конфигурация](#15-конфигурация)
16. [Заметки на будущее (TODO в коде)](#16-заметки-на-будущее-todo-в-коде)
...
| — | Авторизация чтения файлов | **Политика `POL-002 Media Access Policy`** — единый источник правил «кто может читать». UC-MED-002, 003, 004 ссылаются на политику |
| — | Авторизация модификации файлов | **Политика `POL-003 Media Ownership Policy`** — Owner OR Admin. UC-MED-005 ссылается на политику. Системные файлы (`OwnerUserId IS NULL`) — нельзя удалить через UC-MED-005, только через UC-MED-102 |
| — | Принцип разграничения логики авторизации | **Media знает о владении файлом и категории данных, но не о бизнес-семантике**. Например, для RecipeStep правила «Premium-доступ» формируются на стороне Media (через POL-002), но проверка «является ли actor автором блюда» — делегируется в Dishes через `IDishOwnershipService` |
...
## 5. Жизненный цикл `MediaFile`

`MediaFile` проходит через несколько состояний в зависимости от действий пользователя и фоновых задач.

### 5.1. Граф состояний

```
       ┌──────────┐  upload     ┌────────────┐ thumb gen   ┌─────────┐
       │  (нет)   │ ──────────→ │  Uploaded  │ ──────────→ │ Processing │
       └──────────┘              └─────┬──────┘             └────┬────┘
                                       │ thumb готовы            │ ошибка
                                       │                         │
                                       ▼                         ▼
...
|--------|----------|------------------------|
| `Uploaded` | Файл загружен в хранилище, метаданные созданы. Thumbnails ещё не готовы | После успешного upload и записи в БД |
| `Processing` | Идёт обработка (генерация thumbnails). На Этапе 2 — синхронно, фактически статус мгновенный | (только для асинхронной обработки на Этапе 8+) |
| `Ready` | Файл и все необходимые thumbnails готовы — можно отдавать клиенту | После успешной генерации Medium thumbnail (синхронно в момент upload, Этап 2) |
| `Failed` | Ошибка при обработке (повреждённый файл, переполнение хранилища) | При неудачной генерации thumbnail или валидации |
| `Deleted` | Soft delete — метаданные остались, физический файл будет удалён фоновой задачей UC-MED-211 | После UC-MED-005, UC-MED-102, UC-MED-204, UC-MED-210 |

### 5.3. Дополнительные временные поля

Для управления жизненным циклом `MediaFile` хранит:
...
- **Orphan** — `EntityType IS NULL`. Файл загружен, но не привязан к бизнес-сущности. Имеет `ExpiresAt`. Если за 24 часа не привязан — фоновая задача UC-MED-210 удалит его.
- **Attached** — `EntityType IS NOT NULL`. Файл привязан к Dish.MainImageId, RecipeStep.ImageMediaId и т.п. `ExpiresAt = NULL`, `AttachedAt` заполнен.

Переход «orphan → attached» происходит через `IMediaService.AttachToEntityAsync` (UC-MED-202).
Обратный переход «attached → orphan» — через `DetachFromEntityAsync` (UC-MED-203). При этом `ExpiresAt` снова устанавливается в `now + 24h`.

### 5.5. Связь статуса с доступом для клиента

| Статус | Поведение UC-MED-002 / 003 / 004 |
|--------|-------------------------------------|
| `Uploaded` или `Processing` | 425 Too Early (или 404 — выбор реализации, см. POL-002) |
| `Ready` | Отдаётся согласно правилам POL-002 |
| `Failed` или `Deleted` | 404 Not Found |
...
## 10. Абстракция хранилища `IFileStorage`
...
## 11. Межмодульный контракт `IMediaService`
...
## 12. Авторизация: ссылки на политики

Модуль Media использует две политики авторизации, определяющие правила доступа к файлам.

### 12.1. POL-002 Media Access Policy (чтение)

Определяет, кто может **читать** медиафайлы — оригиналы, миниатюры и метаданные.

**Применяется к:** UC-MED-002 (получение оригинала), UC-MED-003 (получение миниатюры), UC-MED-004 (получение метаданных).

**Краткое правило:**
- `DataCategory = Public` — отдаём всем (включая гостей).
- `DataCategory = Personal` — миниатюры всем; оригинал и метаданные — только авторизованным.
- Файлы в статусах `Deleted` / `Failed` — 404.
- На Этапе 3+ для `EntityType = "RecipeStep"` добавится проверка подписки.

Полный текст политики — см. `POL-002-media-access.md`.

### 12.2. POL-003 Media Ownership Policy (модификация)

Определяет, кто может **модифицировать или удалять** медиафайлы.

**Применяется к:** UC-MED-005 (удаление пользовательского файла), и в будущем — всем UC модификации.

**Краткое правило:**
- `Owner` (`MediaFile.OwnerUserId == ActorUserId`) или `Admin`.
- Системные файлы (`OwnerUserId IS NULL`) — нельзя удалить через UC-MED-005, только через UC-MED-102.
- Файл с заполненным `EntityType` — `MEDIA.STILL_ATTACHED`, нужно сначала открепить.

Полный текст политики — см. `POL-003-media-ownership.md`.

### 12.3. Простая авторизация без политик

Для следующих UC отдельные политики не нужны — авторизация делается атрибутами или прямой проверкой:

- **UC-MED-001 UploadFile** — `[Authorize]` (Authenticated).
- **UC-MED-006 GetMyFiles** — `[Authorize]` + проверка `OwnerUserId == ActorUserId`.
- **UC-MED-101, 102, 103** — `[Authorize(Roles = "Admin")]`.
- **UC-MED-200..204** — внутренние методы `IMediaService`. Авторизация делается на уровне UC модуля-вызывающего (например, в UC-DSH-002 проверяется POL-001, и только потом вызывается `AttachToEntityAsync`). Внутренние проверки в `AttachToEntityAsync` (соответствие `OwnerUserId`) — часть контракта, не политика.

---

## 13. Валидации

### 13.1. Пользовательский upload (`UploadFileCommand`)
...
### 13.2. Системный upload (`UploadSystemFileCommand`, только Admin)
...
### 13.4. Библиотеки для реализации
...
## 14. Структура файлов на диске и в S3
...
## 16. Заметки на будущее (TODO в коде)
...
### 16.2. В `UploadFileCommandHandler.cs`
...
### 16.4. Фоновые задачи (Hosted Services) — Этап 8+
...
### 16.5. Новые таблицы — README.md в `Media.Domain/Entities/`
...
### 17.1. По модулю Media — что отложено на обсуждение
...
### 17.2. Дальнейшая работа над Этапом 2
