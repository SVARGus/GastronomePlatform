...
- **PostgreSQL канонизирует `jsonb` и пересортировывает ключи объекта** (подтверждено инцидентом 2026-07-28): дискриминатор `type`, записанный первым свойством, после сохранения в `jsonb`-колонку перестаёт быть первым, а атрибутный полиморфизм .NET 8 требует его строго первым при чтении — десериализация снепшота падает с `NotSupportedException`, хотя round-trip в памяти проходит. Для snapshot-DTO (`PublishedRecipeIngredientDto`) атрибуты `[JsonPolymorphic]`/`[JsonDerivedType]` заменены кастомным `PublishedRecipeIngredientDtoConverter` (регистрируется в `SnapshotJsonOptions`), который находит `type` в любой позиции; формат записи не изменился. Read-DTO API-ответов (`RecipeIngredientViewDto`) остаются на атрибутах — HTTP-ответ формируется заново и через `jsonb` не проходит. Round-trip-тесты снепшота обязаны **эмулировать канонизацию** (перестановку ключей) — иначе этот класс ошибок не ловится.
